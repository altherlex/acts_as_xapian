...
#
$acts_as_xapian_classes_available = Set.new

module ActsAsXapian

    # FIXME: Deixar a definição do esquema mais transparente
    SCHEMA_DEFAULT = 'glb'
    ...
    def ActsAsXapian.prepare_environment
      ...
      @@stemmer = Xapian::Stem.new( ActsAsXapianI18n.stem_language )
    end
    ...
    class Search < QueryBase
        ...
        def initialize(model_classes, query_string, options = {})
            ...
            self.query_string = ActsAsXapianI18n.transate_query( query_string )
    ...
    class ActsAsXapianJob < ActiveRecord::Base
      if SCHEMA_DEFAULT
        set_table_name "#{SCHEMA_DEFAULT}.acts_as_xapian_jobs"
      end

      protected
      def before_create
        self[:id] = ActsAsXapianJob.maximum(:id).to_i + 1
      end
    end
    ...
    def ActsAsXapian.update_index(flush = false, verbose = false)
        ...
        model_classes = ActsAsXapianJob.find_by_sql("select model from #{ActsAsXapianJob.table_name} group by model").map {|a| a.model.constantize}
    ...
    module InstanceMethods
        ...
        def xapian_mark_needs_index
            model = get_class_name_of_model # self.class.base_class.to_s
            model_id = self.send( "#{self.class.primary_key}" )
            ActiveRecord::Base.transaction do
        ...
        def get_class_name_of_model
          [*( self.class.ancestors.map{|f| f.to_s if f.class.to_s == 'Class' }.to_set & $acts_as_xapian_classes_available )][0].to_s
        end
     end
    ...
    module ActsMethods
        ...
        def acts_as_xapian(options)
            ...
            $acts_as_xapian_classes_available << self.to_s
